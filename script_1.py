...
code_examples.append({
    "filename": "retry_handler.py",
    "purpose": "Implements exponential backoff for rate limiting and errors",
    "key_features": "HTTP 429/5xx handling, configurable retry limits, jitter",
    "code_snippet": """
import time
import random
...
code_examples.append({
    "filename": "jira_client.py",
    "purpose": "Handles Jira REST API interactions with pagination",
    "key_features": "Pagination management, authentication, rate limiting",
    "code_snippet": """
import requests
import os
...
code_examples.append({
    ...
    "purpose": "Manages state persistence for fault tolerance",
    "key_features": "Save/load state, atomic writes, resume capability",
    "code_snippet": """
import json
import os
...
code_examples.append({
    "filename": "jsonl_writer.py",
    "purpose": "Writes transformed data to JSONL format",
    "key_features": "Buffered writing, proper encoding, validation",
    "code_snippet": """
import json
from pathlib import Path
...
code_examples.append({
    ...
    "purpose": "Transforms Jira data to LLM training format",
    "key_features": "Text cleaning, task generation, JSONL formatting",
    "code_snippet": """
import re
...
with open('code_examples.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=['filename', 'purpose', 'key_features', 'code_snippet'])
    writer.writeheader()
    writer.writerows(code_examples)

print(f"Created {len(code_examples)} code example templates")
for ex in code_examples:
    print(f"\n{ex['filename']}: {ex['purpose']}")
